...
void setup () {// the setup attaches the servos, ultrasonic sensor, and serial
 ...
 //servo setup
  ...
  delay(2500);
  verticalRotationServo.write(0*servoMaxDegrees/2+min); //pauses it at the midpoint of max sweep and start
  ...
  xbee.onResponse(printErrorCb, (uintptr_t)(Print*)&DebugSerial);
   for (int a = 0; a < 13; a = a + 1) { //this will change the theta value after ultrasonic measurements have been taken from 0-180 degrees phi
    ...
    for (int pos = 0; pos < 13; pos = pos + 1) {
       if (pos*oneRealDegree*degreesPerStep+min<max) {
          sendPacket(type, sensorID, 84, pos, a,servoMaxDegrees); }}} //should be a,pos everywhere else
    delay(2000);
    //  for (int a = 0; a < 13; a = a + 1) { //this will change the theta value after ultrasonic measurements have been taken from 0-180 degrees phi
    // if (a*oneRealDegree*degreesPerStep+min<max) 
    // for (int pos = 0; pos < 13; pos = pos + 1) {
    //    if (pos*oneRealDegree*degreesPerStep+min<max) {
    //       sendPacket(type, sensorID, 48, pos, a,servoMaxDegrees); }}} //should be a,pos everywhere else
    //  delay(2000);
  ...
  ultrasonicScanWithDummy(trigPin, echoPin, horizontalRotationServo, verticalRotationServo, baseline1,baselineDummy, timeToSharpen);
  //printArray(baseline1, "baseline1");
  ...
  //ultrasonicScanWithDummy(trigPin, echoPin, ServoTimer2, baseline[13][13], uint8_t baselineDummy[13][13], numberOfRecursions )
       ...
       if (pos*oneRealDegree*degreesPerStep+min<max) {
         ...
         if ( (deviatesEnough(a, pos, scan, baseline1) == true) //makes sure there is 20% deviation from all baselines
           ...
           && (scan[ a ][ pos ] < 150 ) ) //range is approximately 150in. Anything over this cannot be a detection
          {
             ...
             digitalWrite(blueLED, ledON);//visually show a detection
             sendPacket(type, sensorID, scan[a][pos], a, pos, servoMaxDegrees); //change 0 to the letter pos when doing 3d testing
     ...
     for (int pos = 0; pos < 13; pos = pos + 1) { //scans the 180 degrees of phi at a corresponding theta
       if (pos*oneRealDegree*degreesPerStep+min<max) {
         ...
         delay(delay1);
          ...
          sendPacket(type, sensorID, baseline[a][pos], a, pos,servoMaxDegrees);
...
int ultrasonicScanWithDummy(int trigPin, int echoPin, ServoTimer2, ServoTimer2, uint8_t baseline[13][13], uint8_t baselineDummy[13][13], int numberOfRecursions ) { //this scans the area with an ultrasonic sensor and makes a 13x13 array "mapping" of the surrounding area for baseline detection
  for (int a = 0; a < 13; a = a + 1) { //this will change the theta value after ultrasonic measurements have been taken from 0-180 degrees phi
    if (a*oneRealDegree*degreesPerStep+min<max) {
     horizontalRotationServo.write(a*oneRealDegree*degreesPerStep+min);// why 15?? because that is the angle of the ultrasonic sensore
     delay(delay1);
  ...
  printArray(baseline1, "baseline1");
  for (int timeToSharpen=0; timeToSharpen<numberOfRecursions+1;timeToSharpen=timeToSharpen+1) { //numbver of times baseline is cycled to get the nearest measurement
    for (int a = 0; a < 13; a = a + 1) { //this will change the theta value after ultrasonic measurements have been taken from 0-180 degrees phi
      if (a*oneRealDegree*degreesPerStep+min<max) {
      horizontalRotationServo.write(a*oneRealDegree*degreesPerStep+min);// why 15?? because that is the angle of the ultrasonic sensore
      delay(delay1);
        ...
        if (pos*oneRealDegree*degreesPerStep+min<max) {
         ...
         //  digitalWrite(yellowLED, ledON);
          ...
          //  digitalWrite(yellowLED, ledOFF);
            ...
            digitalWrite(yellowLED, ledOFF);
            if (baselineDummy[a][pos]<6) {
              baselineDummy[a][pos]=169;
            }
            baseline[a][pos]=min(baseline[a][pos],baselineDummy[a][pos]);
            sendPacket(type, sensorID, baseline[a][pos], a, pos, servoMaxDegrees);  //should be a,pos everywhere else
            //sendPacket(type, sensorID, baseline[a][pos], a, pos, servoMaxDegrees);
            //Serial.println(measurement); Serial.println(pos); Serial.println(a); Serial.println(baseline1[a][pos/15]);
            // Serial.print("baseline1, ");Serial.print(a+1);Serial.print(", ");Serial.print(pos/15+1);Serial.print(", ");Serial.println(measurement);
          }
      ...
      Serial.print("baseline 1 modification no.:"); Serial.println(timeToSharpen+1);
      printArray(baseline1, "baseline1 modified");
...
int takeUltrasonicMeasurement() {
  int g=0;
  delay(45);                    // Wait 50ms between pings (about 20 pings/sec). 29ms should be the shortest delay between pings.
 ...
 while (distance==0 && g<10){
  ...
  if (distance<6) {distance=169;}
  if (distance>72) {distance=169;}
...
int sendPacket(int type, int sensorID, int distance, int theta,int phi, int servoMaxDegrees) { //, int servoMaxDegrees
   ...
   //Serial.println("attempt to send packet");
    ...
    verticalRotation=verticalRotation -servoMaxDegrees/2; 
    horizontalRotation=-1*horizontalRotation; //centering the sensor in the GUI as 0 degrees in the Arduino output
    ...
    packetSanic[3]=verticalRotation;
    packetSanic[4]=horizontalRotation;
    //Serial.print(packetSanic[0]);Serial.print(",");Serial.print(packetSanic[1]);Serial.print(",");Serial.print(packetSanic[2]);Serial.print(",");Serial.print(packetSanic[3]);Serial.print(",");Serial.println(packetSanic[4]);
...
bool deviatesEnough(int a, int pos, uint8_t scan[13][13], uint8_t baseline[13][13]) {
 double enough=max(.1 * baseline[a][pos],8);
   if ( abs( scan[a][pos] - baseline[a][pos] ) > enough ) {}
   else {
